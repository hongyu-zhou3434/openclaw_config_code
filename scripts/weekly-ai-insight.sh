...
OUTPUT_DIR="$WORKSPACE/output/weekly-insights/$(date +%Y-%m-%d)"
LOG_FILE="$OUTPUT_DIR/insight-generation.log"
DATE_STR=$(date +%Y%m%d)
DATE_CN=$(date +%Y年%m月%d日)
WEEK_START=$(date -d "6 days ago" +%Y-%m-%d 2>/dev/null || date -v-6d +%Y-%m-%d 2>/dev/null || echo "$(date +%Y-%m-%d)")
WEEK_END=$(date +%Y-%m-%d)
...
echo "=== 每周AI技术洞察报告生成 ===" | tee "$LOG_FILE"
echo "报告周期: $WEEK_START 至 $WEEK_END" | tee -a "$LOG_FILE"
echo "生成日期: $DATE_CN" | tee -a "$LOG_FILE"
...
tavily_search() {
    ...
    while [ $attempt -le $max_retries ]; do
        result=$(TAVILY_API_KEY="$TAVILY_API_KEY" node "$WORKSPACE/skills/tavily-search/scripts/search.mjs" "$query" 2>&1)
        local exit_code=$?
        
        # 检查是否成功
        if [ $exit_code -eq 0 ] && [[ "$result" != *"error"* ]] && [[ "$result" != *"Error"* ]] && [ -n "$result" ]; then
            echo "$result"
            return 0
        fi
        
        # 检查是否是速率限制错误
        if [[ "$result" == *"rate limit"* ]] || [[ "$result" == *"Rate limit"* ]] || [[ "$result" == *"429"* ]]; then
            echo "  ⏱️ 遇到速率限制，等待 ${retry_delay} 秒后重试 ($attempt/$max_retries)..." | tee -a "$LOG_FILE"
            sleep $retry_delay
            # 增加延迟时间
            retry_delay=$((retry_delay + 5))
        else
            # 其他错误，直接返回
            echo "  ⚠️ 搜索失败: $result" | tee -a "$LOG_FILE"
            echo "**搜索失败**: API错误"
            return 1
        fi
    ...
    # 重试次数用完
    echo "  ❌ 搜索失败: 重试次数已用完" | tee -a "$LOG_FILE"
    echo "**搜索失败**: 重试次数已用完"
    return 1
}
...
EOF
    ...
    # 使用Tavily搜索本周最新动态（带延迟和重试）
    echo "  🔍 搜索: ${company_name} AI 模型 发布..." | tee -a "$LOG_FILE"
    local search_result
    search_result=$(search_with_delay "${company_name} AI 模型 发布")
...
EOF
    ...
    search_result=$(search_with_delay "${company_name} 技术突破")
    echo "$search_result" >> "$output_md"
...
基于本周检索结果分析${company_name}在以下方面的进展：
...
EOF
        ...
        search_result=$(search_with_delay "${company_name} ${area_name} ${area_keywords}")
        echo "$search_result" >> "$output_md"
    done
...
convert_to_pdf() {
    ...
    # 收集所有Markdown文件
    local md_files=()
    local summary_md="$OUTPUT_DIR/每周AI洞察汇总_${DATE_STR}.md"
...
send_email_notification() {
    ...
    # 构建PDF附件列表
    local attachments=""
    local summary_pdf="$OUTPUT_DIR/每周AI洞察汇总_${DATE_STR}.pdf"
    ...
    python3 "$WORKSPACE/scripts/send_email_with_progress.py" \
        --to "$QQ_EMAIL" \
        --subject "📊 每周AI技术洞察报告 - ${WEEK_START}至${WEEK_END}" \
        --body "您好！
...
EOF
    ...
    # 搜索本周热点（带延迟和重试）
    echo "  🔍 搜索本周AI热点..." | tee -a "$LOG_FILE"
    local hot_news
    hot_news=$(search_with_delay "AI 大模型 发布")
...
EOF
    ...
    hot_news=$(search_with_delay "AI 技术突破")
    echo "$hot_news" >> "$summary_md"
...
EOF
    ...
    echo "  ✓ 汇总报告已生成: 每周AI洞察汇总_${DATE_STR}.md" | tee -a "$LOG_FILE"
}
...
find "$WORKSPACE/output/weekly-insights" -type d -mtime +30 -exec rm -rf {} + 2>/dev/null || true
...
if [ -d "$WORKSPACE/.git" ]; then
    ...
    git commit -m "每周AI洞察报告 ${WEEK_START}至${WEEK_END}" 2>/dev/null || true
