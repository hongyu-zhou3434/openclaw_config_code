...
当前会话处于非交互模式，工具权限受限。我需要读取项目文件才能进行分析。

请选择以下方式之一来授权：

1. **临时绕过权限检查**：重新运行命令时添加 `-y` 参数
   ```
   codebuddy -y "<原提示内容>"
   ```

2. **永久允许工具**：在设置中将 `Read`、`Glob`、`Bash` 添加到 `permissions.allow` 列表

授权后，我将能够读取 `/tmp/code-analysis/Hello-World` 目录中的文件并完成分析。
